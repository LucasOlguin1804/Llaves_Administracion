...
router.post('/', async (req, res) => {
  const {
    user_id, classroom_id, schedule_id, needs_data_control, notes
  } = req.body;
  ...
  try {
    ...
    const returnTime = `${today} ${scheduleRow[0].end_time}`;
    ...
    await db.query(`
      ...
      VALUES (?, ?, ?, ?, 0, NOW())`,
      [1, 'Nueva solicitud de llave', `El docente con ID ${user_id} ha solicitado una llave.`, 'key_request']
    );
...
router.put('/:id/approve', async (req, res) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    io.emit('solicitud-actualizada', { request_id: id, estado: 'approved' });
...
router.put('/:id/complete', async (req, res) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    io.emit('solicitud-actualizada', { request_id: id, estado: 'completed' });
...
router.post('/manual', async (req, res) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    io.emit('solicitud-actualizada', { request_id: result.insertId, estado: estado || 'approved' });
