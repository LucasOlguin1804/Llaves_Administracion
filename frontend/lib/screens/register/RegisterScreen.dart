...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  bool correoEsValido(String correo) {
    correo = correo.trim().toLowerCase();
    if (!correo.endsWith('@univalle.edu')) return false;
    final nombreParte = correo.split('@')[0];
    if (nombreParte.length > 40) return false;
    return true;
  }
  ...
  Future<void> _register() async {
    ...
    if (!correoEsValido(email)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("El correo debe ser @univalle.edu y tener hasta 40 caracteres antes del @")),
      );
      return;
    }
