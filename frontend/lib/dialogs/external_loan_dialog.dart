...
class _ExternalLoanDialogState extends State<ExternalLoanDialog> {
  ...
  Future<void> _fetchKeyForClassroom(String classroomId) async {
    try {
      final res = await http.get(Uri.parse('http://localhost:3000/api/keys?classroom_id=$classroomId'));
  ...
  Future<void> _submitLoan() async {
    if (!_formKey.currentState!.validate() || _associatedKeyId == null || _selectedClassroomId == null) {
    ...
    final userId = Provider.of<AppState>(context, listen: false).currentUser?.id;
    if (userId == null) {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Préstamo registrado con éxito')));
      } else {
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: Text('Fecha devolución: ${DateFormat('yyyy-MM-dd').format(_expectedReturnTime)}'),
                  ),
                  TextButton(
                    onPressed: () async {
              ...
              Row(
                children: [
                  Expanded(
                    child: Text('Hora devolución: ${DateFormat('HH:mm').format(_expectedReturnTime)}'),
                  ),
                  TextButton(
                    onPressed: () async {
                      final pickedTime = await showTimePicker(
                        context: context,
                        initialTime: TimeOfDay.fromDateTime(_expectedReturnTime),
                      );
                      if (pickedTime != null) {
                        setState(() {
                          _expectedReturnTime = DateTime(
